...
col1.metric("📽️ Total films", f"{len(df_full):,}")
...
st.info(f"""
📌 **Lecture** : le dataset s'étale de **{int(df_full['title_year'].min())}** à 
**{int(df_full['title_year'].max())}**, avec une concentration sur les années 2000-2015. 
Médiane : {int(df_full['title_year'].median())}.
""")
...
with col_g1:
    st.success(f"""
    **🥇 Top 3 des genres**
    1. **{genres_count.index[0]}** — {genres_count.values[0]:,} films
    2. **{genres_count.index[1]}** — {genres_count.values[1]:,} films
    3. **{genres_count.index[2]}** — {genres_count.values[2]:,} films
    """)
with col_g2:
    st.info("""
    📌 **Insight** : Drama domine largement, suivi de Comedy et Action. 
    Ces 3 genres sont **parfaitement alignés** avec les préférences 
    du public sénior creusois identifiées dans l'étude de marché.
    """)
...
col_p2.metric("🇬🇧 Royaume-Uni", f"{(df_full['country'] == 'UK').sum() / df_full['country'].notna().sum() * 100:.1f}%")
col_p3.metric("🇫🇷 France", f"{pct_fr:.1f}%")

st.warning(f"""
⚠️ **Limite identifiée** : {pct_usa:.0f} % des films sont américains. 
C'est un **biais important** à mentionner devant le jury : nos recommandations 
seront fortement orientées vers le cinéma US, ce qui ne correspond pas parfaitement 
aux préférences du public creusois (43,5 % des entrées seniors sur films français).
""")
...
st.markdown("""
**📌 Insights clés issus de la heatmap :**

- **`gross` ↔ `num_voted_users` (0,64)** : les films à gros box-office attirent plus de votes
- **`imdb_score` ↔ `num_voted_users` (0,43)** : les bons films attirent plus d'avis
- **`title_year` ↔ `imdb_score` (-0,21)** : les films **plus récents ont des notes légèrement plus basses** (effet nostalgie)
- **`budget` ↔ `imdb_score` (0,03)** : ⚠️ **AUCUNE corrélation** entre budget et qualité — un gros budget ne fait pas un bon film !
""")
...
with col_act:
    ...
    ax7.barh(top_act.index[::-1], top_act.values[::-1], color='#E8A33A', edgecolor='white')
...
# ===============================
st.header("📊 7. Comparaison : tous les films vs Top 1000 enrichis")
...
axes[0].hist(df_enriched['imdb_score'], bins=30, color='#E8A33A', alpha=0.8, label=f'Top 1000')
...
axes[1].hist(df_enriched['title_year'], bins=40, color='#E8A33A', alpha=0.8, label=f'Top 1000')
...
st.success("""
✅ **Validation** : le top 1000 enrichi affiche une note moyenne **nettement supérieure** 
au dataset complet (+0,8 point), confirmant la qualité de notre sélection pour le 
